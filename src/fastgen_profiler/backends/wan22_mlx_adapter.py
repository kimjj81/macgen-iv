...
class Wan22MLXPipeline:
    ...
    def _fail_if_runtime_failed(self, phase: str) -> None:
        ...
        raise RuntimeMemoryAbort(
            f"Runtime memory abort [wan2.2 {phase}]: pipeline is unusable after "
...
def _assert_bounded_json_structure(value: Any, *, label: str) -> None:
    ...
    while stack:
        ...
        if depth > _MAX_CONFIG_JSON_DEPTH:
            _raise_runtime_abort(
                f"Wan2.2 {label} JSON nesting exceeds safe depth {_MAX_CONFIG_JSON_DEPTH}; "
        ...
        if isinstance(current, dict):
            ...
            if visited_items > _MAX_CONFIG_JSON_ITEMS:
                _raise_runtime_abort(
                    f"Wan2.2 {label} JSON structure exceeds safe item limit "
